resource "aws_instance" "davidluke-docker-registry" {
  ...
  tags = {
    Name = "davidluke-docker-registry"
  }
}
...
resource "null_resource" "buildimages" {
  triggers = {
    first_trigger  = aws_route53_record.dockerregistry-davidluke.ttl
	second_trigger = aws_instance.davidluke-docker-registry.public_ip
  }
  ...
  provisioner "remote-exec" {
    ...
    host     = aws_instance.davidluke-docker-registry.public_ip
  }
