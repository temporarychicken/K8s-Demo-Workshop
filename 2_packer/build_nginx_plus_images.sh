...
sudo docker run -d   --restart=always   --name registryY   -v "$(pwd)"/certs:/certs   -e REGISTRY_HTTP_ADDR=0.0.0.0:443 -e REGISTRY_HTTP_TLS_CERTIFICATE=certs/ws1.nginxdemo.net.fullchain.pem   -e REGISTRY_HTTP_TLS_KEY=certs/ws1.nginxdemo.net.key.pem  -p 443:443  registry:2
...
#sudo sh -c 'echo "127.0.0.1 dockerregistry.ws1.nginxdemo.net">>/etc/hosts'

sudo docker tag $NPLUS_IMAGE dockerregistry.ws1.nginxdemo.net/nginxplus
sudo docker push dockerregistry.ws1.nginxdemo.net/nginxplus
